    ...
    <div class="zone-wrap">
      ...
      <div class="zone-box">
        ...
        <div class="zone-photos" v-show="pageData.currentTab === 'myPhotos'">
          <div class="photos-zone-scroll">
                <div class="addPhotosBtn" v-if="!pageData.picNodata"><vue-core-image-upload
              ref="uploader"
                ...
                <v-scroll v-else class="photos-list" ref="myPhotos" :loadMore="true" :loadstatus="pageData.loadstatus" @scrollEndHandle="loadMorePhotos">
                  <div slot="scrolllist">
                    <div class="photos-box" v-for="(item, index) in pageData.picArr">
                      <div class="box-bg">
                        <div class="box-list">
                          <div class="photos-item" v-for="(pitem, pindex) in item.detail" v-if="pindex < 3" @click.prevent.stop="linkMyPhotos(item.id, pindex, item)"
                               :style="'background:url(' + pitem + ') no-repeat center; background-size: cover;'">
                          </div>
                        </div>
                        <div class="box-info">
                          <div class="box-avator"><img :src="item.headimgurl"></div>
                          <div class="box-title">
                            <h2>{{item.username}}</h2>
                            <p>{{item.time}}发布</p>
                          </div>
                          <div class="box-bar">
                            ...
                            <div :class="'tip-thumbup ' + (item.support_flag ? 'parised' : '')" @click.prevent.stop="praisePic(index, item.id, item.support_flag, item.support_num, $event)">
                              <div class="baritem">
    ...
    export default {
      ...
      data () {
        return {
          ...
          pageData: {
            ...
            username: '',
            avator: '',
            navIndex: this.$store.state.zoneIndex,
      ...
      methods: {
        ...
        async getUserTravelNote (index) {
          try {
            let response = await getTravelNote(
              {
                pageNow: index,
                pageSize: 5
              }
        ...
        async praisePic (index, id, flag, num, e) {
          if (!e._constructed) {
            return;
          }
          try {
            let n = flag ? -1 : 1;
            let status = flag ? !1 : !0;
            let resp = await thumbUpPhotos({
              id: id
            });
            this.pageData.picArr[index].support_flag = status;
            this.pageData.picArr[index].support_num = parseInt(num + n);
            this.$store.state.ucInfo.myscore = resp.data.data.score;
          } catch (e) {
            this.showMessage('网络连接错误，请重试！');
          }
        ...
        async requestPic (index) {
          try {
            let piclist = await inquirePhotos({
              pageNow: index,
              pageSize: 5
            });
        ...
        async getUserFeed (index) {
          try {
            ...
            this.$nextTick(() => {
              this.$refs.feedback && this.$refs.feedback.refresh();
      ...
      .zone-photos
        ...
        .photos-zone-scroll
          ...
          .addPhotosBtn
            width px2rem(157)
            height px2rem(157)
            background url("../../assets/image/addPhoto-btn.png") no-repeat
            background-size 100% 100%
            position absolute
            z-index 999
            right px2rem(10)
